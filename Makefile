...
###                             Project Info                                 ###
################################################################################
PROJECT_NAME := grbc# unique namespace for project
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=grbc \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=grbc \
...
build: go.sum
ifeq ($(OS), Windows_NT)
	$(GO_BIN) build -mod=readonly $(BUILD_FLAGS) -o out/$(shell $(GO_BIN) env GOOS)/grbc.exe ./cmd/grbc
else
	$(GO_BIN) build -mod=readonly $(BUILD_FLAGS) -o out/$(shell $(GO_BIN) env GOOS)/grbc ./cmd/grbc
endif

build-linux: go.sum
	LEDGER_ENABLED=false GOOS=linux GOARCH=amd64 $(MAKE) build

# build on rocksdb-backed grbc on macOS with shared libs from brew
...
install: go.sum
	$(GO_BIN) install -mod=readonly $(BUILD_FLAGS) ./cmd/grbc
...
# Build docker image and tag as grbc/grbc:local
docker-build:
	DOCKER_BUILDKIT=1 $(DOCKER) build -t grbc/grbc:local .

docker-build-rocksdb:
	DOCKER_BUILDKIT=1 $(DOCKER) build -f Dockerfile-rocksdb -t grbc/grbc:local .
...
localnet-start: build-linux localnet-stop
	@if ! [ -f build/node0/kvd/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/kvd:Z grbc/grbcnode testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	$(DOCKER) compose up -d
...
test-ibc: docker-build
	cd tests/e2e-ibc && GRBC_TAG=local $(GO_BIN) test -timeout 10m .
.PHONY: test-ibc
...
start-remote-sims:
	...
	docker build -f simulations/Dockerfile -t grbc/grbc-sim:master .
	# push that image to the hub
	docker push grbc/grbc-sim:master
	...
	aws batch submit-job \
		...
		-—job-definition grbc-sim-master \

...
$BINARY add-genesis-account $validatorKeyName 2000000000ugrbc,100000000000bnb
...
$BINARY add-genesis-account $faucetKeyName 1000000000ugrbc,100000000000bnb

evmFaucetKeyName="evm-faucet"
printf "$evmFaucetMnemonic\n" | $BINARY keys add $evmFaucetKeyName --eth --recover
$BINARY add-genesis-account $evmFaucetKeyName 1000000000ugrbc

userKeyName="user"
printf "$userMnemonic\n" | $BINARY keys add $userKeyName --eth --recover
$BINARY add-genesis-account $userKeyName 1000000000ugrbc,1000000000usdx

relayerKeyName="relayer"
printf "$relayerMnemonic\n" | $BINARY keys add $relayerKeyName --eth --recover
$BINARY add-genesis-account $relayerKeyName 1000000000ugrbc

# Create a delegation tx for the validator and add to genesis
$BINARY gentx $validatorKeyName 1000000000ugrbc --keyring-backend test --chain-id $chainID
$BINARY collect-gentxs

# Replace stake with ugrbc
sed -in-place='' 's/stake/ugrbc/g' $DATA/config/genesis.json

# Replace the default evm denom of aphoton with ugrbc
sed -in-place='' 's/aphoton/agrbc/g' $DATA/config/genesis.json

...
func newShardCmd(opts ethermintserver.StartOptions) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `shard opens a local GriubCoin home directory's databases and removes all blocks outside a range defined by --start and --end. The range is inclusive of the end block.
...
WARNING: this is a destructive action.`,
		Example: `Create a 1M block data shard (keeps blocks GriubCoin 1,000,000 to 2,000,000)
$ grbc shard --home path/to/.griubcoin --start 1000000 --end 2000000

Prune all blocks up to 5,000,000:
$ grbc shard --home path/to/.griubcoin --start 5000000 --end -1

Prune first 1M blocks _without_ affecting blockstore or cometBFT state:
$ grbc shard --home path/to/.griubCoin --start 1000000 --end -1 --only-app-state`,

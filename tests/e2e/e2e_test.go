...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/tests/e2e/testutil"
	"github.com/GriubCoin-Network/GriubCoin/tests/util"
)

var (
	minEvmGasPrice = big.NewInt(1e10) // agrbc
)

func ugrbc(amt int64) sdk.Coin {
	return sdk.NewCoin("ugrbc", sdkmath.NewInt(amt))
}
...
func (suite *IntegrationTestSuite) TestFundedAccount() {
	...
	res, err := suite.Kava.Grpc.Query.Bank.Balance(context.Background(), banktypes.NewQueryBalanceRequest(
		acc.SdkAddress, "ugrbc",
	))
	suite.NoError(err)
	suite.Equal(funds, *res.Balance)

	// check balance via EVM query
	agrbcBal, err := suite.Kava.EvmClient.BalanceAt(context.Background(), acc.EvmAddress, nil)
	suite.NoError(err)
	suite.Equal(funds.Amount.MulRaw(1e12).BigInt(), agrbcBal)
}
...
func (suite *IntegrationTestSuite) TestTransferOverEVM() {
	...
	kavaToTransfer := big.NewInt(1e17) // .1 KAVA; agrbc has 18 decimals.
	...
	ugrbcUsedForGas := sdkmath.NewIntFromBigInt(minEvmGasPrice).
		Mul(sdkmath.NewIntFromUint64(res.Receipt.GasUsed)).
		QuoRaw(1e12) // convert agrbc to ugrbc

	// expect (9 - gas used) KAVA remaining in account.
	balance := suite.Kava.QuerySdkForBalances(acc.SdkAddress)
	suite.Equal(sdkmath.NewInt(9e5).Sub(ugrbcUsedForGas), balance.AmountOf("ugrbc"))
}
...
func (suite *IntegrationTestSuite) TestIbcTransfer() {
	...
	fundsToSend := ugrbc(5e4) // .005 KAVA
	...
	suite.Eventually(func() bool {
		...
		return balance.AmountOf("ugrbc").Equal(expectedSrcBalance.Amount)

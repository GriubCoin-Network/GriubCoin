...
func (suite *IntegrationTestSuite) TestEthGasPriceReturnsMinFee() {
	...
	// evm uses agrbc, get agrbc min fee
	evmMinGas := minGasPrices.AmountOf("agrbc").TruncateInt().BigInt()
...
func (suite *IntegrationTestSuite) TestEvmRespectsMinFee() {
	...
	sender := suite.Kava.NewFundedAccount("evm-min-fee-test-sender", sdk.NewCoins(ugrbc(1e3)))
	...
	suite.NoError(err)
	minGasPrice := minFees.AmountOf("agrbc").TruncateInt()

...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/tests/e2e/contracts/greeter"
...
func (suite *IntegrationTestSuite) TestEthCallToGreeterContract() {
	...
	user := suite.Kava.NewFundedAccount("greeter-contract-user", sdk.NewCoins(ugrbc(1e6)))
...
func (suite *IntegrationTestSuite) TestEip712BasicMessageAuthorization() {
	...
	sender := suite.Kava.NewFundedAccount("eip712-msgSend", sdk.NewCoins(ugrbc(2e4)))
	receiver := app.RandomAddress()

	// setup message for sending some kava to random receiver
	msgs := []sdk.Msg{
		banktypes.NewMsgSend(sender.SdkAddress, receiver, sdk.NewCoins(ugrbc(1e3))),
	}
...
func (suite *IntegrationTestSuite) TestEip712ConvertToCoinAndDepositToLend() {
	...
	// create new funded account
	depositor := suite.Kava.NewFundedAccount("eip712-lend-depositor", sdk.NewCoins(ugrbc(1e5)))
	...
	withdrawAndConvertBack := util.KavaMsgRequest{
		...
		GasLimit:  1e6,
		FeeAmount: sdk.NewCoins(ugrbc(1000)),

...
func DefaultKavaChainConfig(chainId string) ibc.ChainConfig {
	...
	genesis := []cosmos.GenesisKV{
		...
		cosmos.NewGenesisKV("app_state.evm.params.evm_denom", "agrbc"),
		// update for fast voting periods
		cosmos.NewGenesisKV("app_state.gov.params.voting_period", "10s"),
		cosmos.NewGenesisKV("app_state.gov.params.min_deposit.0.denom", "ugrbc"),
	}
	...
	return ibc.ChainConfig{
		...
		Bech32Prefix:          "grbc",
		Denom:                 "ugrbc",
		GasPrices:             "0ugrbc", // 0 gas price makes calculating expected balances simpler
		GasAdjustment:         1.5,

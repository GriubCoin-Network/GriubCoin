...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/savings/keeper"
...
func (suite *invariantTestSuite) SetupValidState() {
	depositAmt := sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(2e8)))
...
func (suite *invariantTestSuite) TestSolvencyInvariant() {
	...
	suite.keeper.SetDeposit(suite.ctx, types.NewDeposit(
		suite.addrs[0],
		sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(3e8))),
	))
	...
	suite.keeper.SetDeposit(suite.ctx, types.NewDeposit(
		suite.addrs[0],
		sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(1e8))),
	))

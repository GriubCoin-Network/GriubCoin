...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/savings/types"
)
...
func (suite *KeeperTestSuite) TestWithdraw() {
	...
	testCases := []withdrawTest{
		{
			...
			args{
				allowedDenoms:             []string{"bnb", "btcb", "ugrbc"},
		...
		},
		{
			"valid: partial bkava",
			args{
				allowedDenoms:             []string{"bnb", "btcb", "ugrbc", "bkava"},
				depositor:                 sdk.AccAddress(crypto.AddressHash([]byte("test"))),
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.CreateAccountWithAddress(valAccAddr, cs(c("ugrbc", 100e10)))
			suite.CreateAccountWithAddress(delegator, cs(c("ugrbc", 100e10)))

...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/earn"
...
func (suite *genesisTestSuite) Test_InitAndExportGenesis() {
	...
	state := types.NewGenesisState(
		...
		types.VaultRecords{
			types.VaultRecord{
				TotalShares: types.NewVaultShare("ugrbc", sdk.NewDec(3800000)),
			},
			types.VaultRecord{
		...
		types.VaultShareRecords{
			types.VaultShareRecord{
				...
				Shares: types.NewVaultShares(
					...
					types.NewVaultShare("ugrbc", sdk.NewDec(1900000)),
				),
			},
			types.VaultShareRecord{
	...
	vaultRecord1, _ := suite.Keeper.GetVaultRecord(suite.Ctx, "ugrbc")

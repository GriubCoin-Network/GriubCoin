...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/earn/testutil"
...
func (suite *grpcQueryTestSuite) TestDeposits() {
	...
	startBalance := sdk.NewCoins(
		sdk.NewCoin("ugrbc", initialUkavaBalance),
	...
	suite.Run("specific bkava vault in staked tokens", func() {
		...
		suite.Require().Equal(
			types.DepositResponse{
				...
				// Only the specified vault denom value
				Value: sdk.NewCoins(
					sdk.NewCoin("ugrbc", deposit3Amount.Amount),
				),
			},
			res.Deposits[0],
		)
	...
	suite.Run("all vaults value in staked tokens", func() {
		...
		suite.Require().Equal(
			types.DepositResponse{
				...
				Value: sdk.Coins{
					// Does not include non-bkava vaults
					sdk.NewCoin("ugrbc", deposit4Amount.Amount),
					sdk.NewCoin("ugrbc", deposit3Amount.Amount),
				},
			},
			res.Deposits[0],
...
func (suite *grpcQueryTestSuite) TestDeposits_bKava() {
	...
	err := suite.App.FundAccount(suite.Ctx, address1, sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(1e9))))
	suite.Require().NoError(err)
	...
	err = suite.Keeper.Deposit(suite.Ctx, address1, sdk.NewInt64Coin("ugrbc", 1e6), types.STRATEGY_TYPE_SAVINGS)
	suite.Require().NoError(err)

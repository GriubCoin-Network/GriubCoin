...
func (s queryServer) getOneAccountOneVaultDeposit(
	...
	if req.ValueInStakedTokens {
		// Get underlying ugrbc amount if denom is a derivative
		...
		ugrbcValue, err := s.keeper.liquidKeeper.GetStakedTokensForDerivatives(ctx, sdk.NewCoins(value))
		if err != nil {
			// This should "never" happen if IsDerivativeDenom is true
			panic("Error getting ugrbc value for " + req.Denom)
		}
...
func (s queryServer) getOneAccountAllDeposits(
	...
	if req.ValueInStakedTokens {
		// Plain slice to not sum ugrbc amounts together. This is not a valid
		...
		for _, coin := range value {
			...
			ugrbcValue, err := s.keeper.liquidKeeper.GetStakedTokensForDerivatives(ctx, sdk.NewCoins(coin))
			if err != nil {
				// This should "never" happen if IsDerivativeDenom is true
				panic("Error getting ugrbc value for " + coin.Denom)
			}
			valueInStakedTokens = append(valueInStakedTokens, ugrbcValue)
		}

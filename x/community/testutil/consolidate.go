...
import (
	...
	types "github.com/GriubCoin-Network/GriubCoin/x/community/types"
...
func (suite *disableInflationTestSuite) TestStartCommunityFundConsolidation() {
	tests := []struct {
	...
	}{
		{
			...
			sdk.NewDecCoins(
				sdk.NewDecCoinFromDec("ugrbc", sdk.NewDecWithPrec(123456, 2)),
				sdk.NewDecCoinFromDec("usdx", sdk.NewDecWithPrec(654321, 3)),
			),
			sdk.NewCoins(
				sdk.NewInt64Coin("ugrbc", 10_000),
	...
	for _, tc := range tests {
		suite.Run(tc.name, func() {
			...
			expectedKavaDistCoins := sdk.NewCoins(sdk.NewCoin("ugrbc", kavaDistCoinsBefore.AmountOf("ugrbc")))
			...
			suite.Run("module balances after consolidation should moved", func() {
				...
				suite.Equal(
					...
					kavaDistCoinsAfter,
					"x/kavadist balance should ony contain ugrbc",
				)

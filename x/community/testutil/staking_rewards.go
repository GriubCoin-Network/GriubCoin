...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/community/types"
)
...
func (suite *stakingRewardsTestSuite) TestStakingRewards() {
	testCases := []struct {
		...
		// the amount of ugrbc to mint and transfer to the community pool
		// to use to pay for rewards
		communityPoolFunds sdkmath.Int

		// how many total rewards are expected to be accumulated in ugrbc
		...
		//	expectedRewardsTotal: sdkmath.NewInt(49999999999999), // truncation results in 1 ugrbc error
		//},
		//
		//
		...
		//
		{
			...
			expectedRewardsTotal: sdkmath.NewInt(136986301369), // 50 million / 365 days  - 1 ugrbc
		...
		{
			name:                 "able to accumulate decimal ugrbc units across blocks",
			periodStart:          time.Date(2023, 1, 1, 0, 0, 0, 0, time.UTC),
			periodEnd:            time.Date(2023, 1, 2, 0, 0, 0, 0, time.UTC),
			blockTimeRangeMin:    5.5,
			blockTimeRangeMax:    6.5,
			rewardsPerSecond:     sdkmath.LegacyMustNewDecFromStr("0.100000000000000000"), // blocks are not long enough to accumulate a single ugrbc with this rate
			communityPoolFunds:   sdkmath.NewInt(10000),
			expectedRewardsTotal: sdkmath.NewInt(8640),
		},
		{
			name:                 "down to 1 ugrbc per year can be accumulated -- we are safe from underflow at reasonably small values",
			periodStart:          time.Date(2023, 1, 1, 0, 0, 0, 0, time.UTC),
			periodEnd:            time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC),
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.Require().True(bankKeeper.GetBalance(ctx, poolAcc.GetAddress(), "ugrbc").Amount.IsZero(), "expected community pool to start with zero coins in test genesis")
			// fund withexact amount from test case
			suite.App.FundAccount(ctx, poolAcc.GetAddress(), sdk.NewCoins(sdk.NewCoin("ugrbc", tc.communityPoolFunds)))
			...
			initialFeeCollectorBalance := bankKeeper.GetBalance(ctx, feeCollectorAcc.GetAddress(), "ugrbc").Amount
			...
			endingFeeCollectorBalance := bankKeeper.GetBalance(ctx, feeCollectorAcc.GetAddress(), "ugrbc").Amount
			...
			if tc.expectedRewardsTotal.IsZero() {
				...
				suite.Equal(
					...
					eventCoins.AmountOf("ugrbc").String(),
			...
			actualCommunityPoolBalance := bankKeeper.GetBalance(ctx, poolAcc.GetAddress(), "ugrbc").Amount
...
func (suite *stakingRewardsTestSuite) TestStakingRewardsDoNotAccumulateWhenPoolIsDrained() {
	...
	app.FundAccount(ctx, poolAcc.GetAddress(), sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(10000000)))) // 10 KAVA
	initialFeeCollectorBalance := bankKeeper.GetBalance(ctx, feeCollectorAcc.GetAddress(), "ugrbc").Amount
	...
	app.FundAccount(ctx, poolAcc.GetAddress(), sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(100000000)))) // 100 KAVA
	...
	rewards := bankKeeper.GetBalance(ctx, feeCollectorAcc.GetAddress(), "ugrbc").Amount.Sub(initialFeeCollectorBalance)

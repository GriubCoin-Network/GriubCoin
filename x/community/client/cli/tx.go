...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/community/client/utils"
...
func getCmdFundCommunityPool() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s fund-community-module 10000000ugrbc --from <key>`, version.AppName, types.ModuleName,
		),
...
Example:
$ %s tx gov submit-proposal community-pool-lend-deposit <path/to/proposal.json> --deposit 1000000000ugrbc --from=<key_or_address>
...
Note that --deposit below is the initial proposal deposit submitted along with the proposal.
Example:
$ %s tx gov submit-proposal community-pool-lend-withdraw <path/to/proposal.json> --deposit 1000000000ugrbc --from=<key_or_address>
Where proposal.json contains:
{

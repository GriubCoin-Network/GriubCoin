...
func ugrbc(amt int64) sdk.Coins {
	return sdk.NewCoins(c("ugrbc", amt))
}
...
func (suite *proposalTestSuite) SetupTest() {
	...
	hardGS, pricefeedGS := testutil.NewLendGenesisBuilder().
		WithMarket("ugrbc", "kava:usd", sdk.OneDec()).
	...
	tApp.InitializeFromGenesisStatesWithTimeAndChainID(
		...
		testutil.NewCDPGenState(tApp.AppCodec(), "ugrbc", "grbc", sdk.NewDec(2)),
	)
	...
	// ugrbc
	suite.FundCommunityPool(ugrbc(2e10))
	// usdx
...
func (suite *proposalTestSuite) TestCommunityLendDepositProposal() {
	testCases := []struct {
	...
	}{
		{
			...
			expectedErr:      "",
			expectedDeposits: []sdk.Coins{ugrbc(1e8)},
		},
		{
			name: "valid - one proposal, multiple denoms",
			proposals: []*types.CommunityPoolLendDepositProposal{
				{Amount: ugrbc(1e8).Add(usdx(1e8)...)},
			},
			expectedErr:      "",
			expectedDeposits: []sdk.Coins{ugrbc(1e8).Add(usdx(1e8)...)},
		},
		{
			...
			expectedErr:      "",
			expectedDeposits: []sdk.Coins{ugrbc(1e8 + 1e9)},
		},
		{
...
func (suite *proposalTestSuite) TestCommunityLendWithdrawProposal() {
	testCases := []struct {
	...
	}{
		{
			...
			initialDeposit: ugrbc(1e9),
			proposals: []*types.CommunityPoolLendWithdrawProposal{
				{Amount: ugrbc(1e12)},
			},
			expectedErr:        "",
			expectedWithdrawal: ugrbc(1e9),
		},
		{
		...
		},
		{
			...
			initialDeposit: ugrbc(1e9).Add(usdx(1e9)...),
			proposals: []*types.CommunityPoolLendWithdrawProposal{
				{Amount: ugrbc(1e9).Add(usdx(1e9)...)},
			},
			expectedErr:        "",
			expectedWithdrawal: ugrbc(1e9).Add(usdx(1e9)...),
		},
		{
			name:           "valid - single proposal, partial withdrawal",
			initialDeposit: ugrbc(1e9).Add(usdx(1e9)...),
			proposals: []*types.CommunityPoolLendWithdrawProposal{
				{Amount: ugrbc(1e8).Add(usdx(1e9)...)},
			},
			expectedErr:        "",
			expectedWithdrawal: ugrbc(1e8).Add(usdx(1e9)...),
		},
		{
		...
		},
		{
			...
			expectedErr:        "",
			expectedWithdrawal: ugrbc(1e8).Add(usdx(1e8)...),
		},
		{
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			// during the test - this is because staking denom is "ugrbc" and no
...
func (suite *proposalTestSuite) TestCommunityCDPRepayDebtProposal() {
	initialModuleFunds := ugrbc(2e10).Add(otherdenom(1e9)...)
	...
	testcases := []struct {
	...
	}{
		{
			...
			initialDebt: &debt{c("ugrbc", 1e10), c("usdx", 1e9)},
	...
	testcases := []struct {
	...
	}{
		{
			...
			proposal: types.NewCommunityCDPWithdrawCollateralProposal(
				...
				collateralType,
				c("ugrbc", 8e9-1), // Withdraw all collateral except 2*principal-1 amount
			),
			expectedErr:       "",
			expectedWithdrawn: c("ugrbc", 8e9-1),
		},
		{
		...
		},
		{
			...
			proposal: types.NewCommunityCDPWithdrawCollateralProposal(
				...
				"description goes here",
				collateralType,
				c("ugrbc", 9e9), // <-- would be under collateralized
			),
			expectedErr:       "proposed collateral ratio is below liquidation ratio",
			expectedWithdrawn: c("ugrbc", 0),
		},
	}

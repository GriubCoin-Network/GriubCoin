	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			authBuilder := app.NewAuthBankGenesisBuilder().
				WithSimpleAccount(suite.addrs[0], cs(c("ugrbc", 1e9))).
				WithSimpleAccount(sdk.AccAddress(suite.validatorAddrs[0]), cs(c("ugrbc", 1e9)))
...
func (suite *DelegatorRewardsTestSuite) TestSynchronizeDelegatorReward() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			selfDelegationCoins := c("ugrbc", 1_000_000)
...
func (suite *DelegatorRewardsTestSuite) TestUnbondingValidatorSyncsClaim() {
	authBuilder := app.NewAuthBankGenesisBuilder().
		...
		WithSimpleAccount(sdk.AccAddress(suite.validatorAddrs[0]), cs(c("ugrbc", 1e9))).
		WithSimpleAccount(sdk.AccAddress(suite.validatorAddrs[1]), cs(c("ugrbc", 1e9))).
		WithSimpleAccount(sdk.AccAddress(suite.validatorAddrs[2]), cs(c("ugrbc", 1e9)))

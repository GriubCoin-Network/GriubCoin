...
func (suite *HandlerTestSuite) TestEarnLiquidClaim() {
	...
	authBuilder := suite.authBuilder().
		WithSimpleAccount(userAddr1, cs(c("ugrbc", 1e12))).
		WithSimpleAccount(userAddr2, cs(c("ugrbc", 1e12))).
		WithSimpleAccount(validatorAddr1, cs(c("ugrbc", 1e12))).
		WithSimpleAccount(validatorAddr2, cs(c("ugrbc", 1e12)))
	...
	// Use ugrbc for mint denom
	mParams := mk.GetParams(suite.Ctx)
	mParams.MintDenom = "ugrbc"
	...
	err := suite.App.FundModuleAccount(suite.Ctx, distrtypes.ModuleName, cs(c("ugrbc", 1e12)))
	suite.NoError(err)

	// Create validators
	err = suite.DeliverMsgCreateValidator(valAddr1, c("ugrbc", 1e9))
	suite.Require().NoError(err)

	err = suite.DeliverMsgCreateValidator(valAddr2, c("ugrbc", 1e9))
	suite.Require().NoError(err)
	...
	// User 1: 1e9 ugrbc to validator 1
	// User 2: 99e9 ugrbc to validator 1 AND 2
	err = suite.DeliverMsgDelegate(userAddr1, valAddr1, c("ugrbc", 1e9))
	suite.Require().NoError(err)

	err = suite.DeliverMsgDelegate(userAddr2, valAddr1, c("ugrbc", 99e9))
	suite.Require().NoError(err)

	err = suite.DeliverMsgDelegate(userAddr2, valAddr2, c("ugrbc", 99e9))
	suite.Require().NoError(err)

	// Mint liquid tokens
	_, err = suite.DeliverMsgMintDerivative(userAddr1, valAddr1, c("ugrbc", 1e9))
	suite.Require().NoError(err)

	_, err = suite.DeliverMsgMintDerivative(userAddr2, valAddr1, c("ugrbc", 99e9))
	suite.Require().NoError(err)

	_, err = suite.DeliverMsgMintDerivative(userAddr2, valAddr2, c("ugrbc", 99e9))
	suite.Require().NoError(err)
	...
	// Claim ugrbc staking rewards
	denomsToClaim := map[string]string{"ugrbc": "large"}
	...
	stakingRewards1 := delegationRewards.
		...
		Quo(sdk.NewDec(100)).
		RoundInt()
	suite.BalanceEquals(userAddr1, preClaimBal1.Add(sdk.NewCoin("ugrbc", stakingRewards1)))
	...
	suite.BalanceInEpsilon(
		userAddr2,
		preClaimBal2.Add(sdk.NewCoin("ugrbc", stakingRewards2)),
		// Highest precision to allow 1ugrbc margin of error
		// 820778117815 vs 820778117814
		1e-11,
	)

	suite.InEpsilonf(
		delegationRewards.AmountOf("ugrbc").RoundInt().Int64(),

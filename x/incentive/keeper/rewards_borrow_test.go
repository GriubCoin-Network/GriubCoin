...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/incentive/keeper"
	"github.com/GriubCoin-Network/GriubCoin/x/incentive/testutil"
	"github.com/GriubCoin-Network/GriubCoin/x/incentive/types"
...
func (suite *BorrowRewardsTestSuite) TestAccumulateHardBorrowRewards() {
	...
	testCases := []test{
		...
		},
		{
			...
			args{
				borrow:           c("bnb", 1000000000000),
				rewardsPerSecond: cs(c("hard", 122354), c("ugrbc", 122354)),
				timeElapsed:      7,
				expectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("0.000000856478000001")),
					types.NewRewardIndex("ugrbc", d("0.000000856478000001")),
				},
			},
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					...
					types.NewRewardIndex("ugrbc", d("0.010571385600010177")),
				},
			},
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					...
					types.NewRewardIndex("ugrbc", d("0.0")),
				},
			},
		},
		{
			...
			args{
				borrow:           c("bnb", 1000000000000),
				rewardsPerSecond: cs(c("hard", 122354), c("ugrbc", 555555)),
				timeElapsed:      86400,
				expectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("0.010571385600010177")),
					types.NewRewardIndex("ugrbc", d("0.047999952000046210")),
				},
			},
		},
	}
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			authBuilder := app.NewAuthBankGenesisBuilder().WithSimpleAccount(
				userAddr,
				cs(c("bnb", 1e15), c("ugrbc", 1e15), c("btcb", 1e15), c("xrp", 1e15), c("zzz", 1e15)),
			)
...
func (suite *BorrowRewardsTestSuite) TestSynchronizeHardBorrowReward() {
	...
	testCases := []test{
		...
		},
		{
			...
			args{
				incentiveBorrowRewardDenom: "ugrbc",
				borrow:                     c("ugrbc", 1), // borrow a tiny amount so that rewards round to zero
				rewardsPerSecond:           cs(c("hard", 122354)),
				blockTimes:                 []int{10, 10, 10, 10, 10, 10, 10, 10, 10, 10},
		...
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					...
					types.NewRewardIndex("ugrbc", d("0.001223540000173228")),
				},
				expectedRewards: cs(c("hard", 12235400), c("ugrbc", 12235400)),
			},
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					...
					types.NewRewardIndex("ugrbc", d("10.571385603126235340")),
				},
				expectedRewards: cs(c("hard", 105713856031), c("ugrbc", 105713856031)),
			},
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("0.001223540000173228")),
					types.NewRewardIndex("ugrbc", d("0.005555550000786558")),
				},
				expectedRewards: cs(c("hard", 12235400), c("ugrbc", 55555500)),
			},
		},
		{
			...
			args{
				...
				updatedRewardsPerSecond:    cs(c("hard", 122354), c("ugrbc", 100000)),
				updatedExpectedRewards:     cs(c("hard", 21142771202), c("ugrbc", 8640000000)),
				updatedExpectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("2.114277120120202320")),
					types.NewRewardIndex("ugrbc", d("0.864000000049120715")),
				},
		...
		},
		{
			...
			args{
				...
				updateRewardsViaCommmittee: true,
				updatedBaseDenom:           "zzz",
				updatedRewardsPerSecond:    cs(c("hard", 100000), c("ugrbc", 100500), c("swap", 500)),
				updatedExpectedRewards:     cs(c("hard", 8640000000), c("ugrbc", 8683200001), c("swap", 43200000)),
				updatedExpectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("0.864000000049803065")),
					types.NewRewardIndex("ugrbc", d("0.868320000050052081")),
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			authBuilder := app.NewAuthBankGenesisBuilder().
				WithSimpleAccount(suite.addrs[2], cs(c("ugrbc", 1e9))).
				WithSimpleAccount(userAddr, cs(c("bnb", 1e15), c("ugrbc", 1e15), c("btcb", 1e15), c("xrp", 1e15), c("zzz", 1e15)))
			...
			suite.Require().NoError(
				suite.hardKeeper.Deposit(suite.ctx, suite.addrs[2], cs(c("ugrbc", 200_000_000))),
			)
			suite.Require().NoError(
				suite.hardKeeper.Borrow(suite.ctx, suite.addrs[2], cs(c("ugrbc", 100_000_000))),
			)
	...
	testCases := []test{
		{
			...
			args{
				...
				modification:              withdrawModification{coins: cs(c("ugrbc", 500000000))},
				rewardsPerSecond:          cs(c("hard", 122354)),
				expectedBorrowIndexDenoms: []string{"bnb", "ugrbc"},
			},
		},
		{
			...
			args{
				...
				modification:              withdrawModification{coins: cs(c("ugrbc", 500000000), c("bnb", 50000000000), c("xrp", 50000000000))},
				rewardsPerSecond:          cs(c("hard", 122354)),
				expectedBorrowIndexDenoms: []string{"btcb", "ugrbc", "bnb", "xrp"},
			},
		},
		{
		...
		},
		{
			...
			args{
				initialDeposit:            cs(c("bnb", 1000000000)),
				firstBorrow:               cs(c("bnb", 100000000), c("ugrbc", 10000000)),
				modification:              withdrawModification{coins: cs(c("bnb", 1100000000)), repay: true},
				rewardsPerSecond:          cs(c("hard", 122354)),
				expectedBorrowIndexDenoms: []string{"ugrbc"},
			},
		},
		{
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			incentBuilder := testutil.NewIncentiveGenesisBuilder().
				...
				WithSimpleBorrowRewardPeriod("ugrbc", tc.args.rewardsPerSecond).
...
func (suite *BorrowRewardsTestSuite) TestSimulateHardBorrowRewardSynchronization() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			userAddr := suite.addrs[3]
			authBuilder := app.NewAuthBankGenesisBuilder().WithSimpleAccount(userAddr, cs(c("bnb", 1e15), c("ugrbc", 1e15), c("btcb", 1e15), c("xrp", 1e15), c("zzz", 1e15)))

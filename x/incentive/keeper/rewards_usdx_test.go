	...
	testCases := []test{
		{
			...
			args{
				ctype:                 "bnb-a",
				rewardsPerSecond:      c("ugrbc", 122354),
	...
	testCases := []test{
		{
			...
			args{
				...
				expectedRewards:      c("ugrbc", 12235400),
			},
		},
		{
			...
			args{
				...
				expectedRewards:      c("ugrbc", 105713856000),
			},
		},
	}
	for _, tc := range testCases {
...
func (suite *USDXRewardsTestSuite) TestSimulateUSDXMintingRewardSynchronization() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.Require().Equal(claim.Reward, sdk.NewCoin("ugrbc", sdk.ZeroInt()))

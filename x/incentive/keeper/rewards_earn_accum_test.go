...
func (suite *AccumulateEarnRewardsTests) TestStateUpdatedWhenBlockTimeHasIncreased_bkava() {
	...
	// Each vault gets the same ugrbc per second, assuming shares prices are the same.
...
func (suite *AccumulateEarnRewardsTests) TestStateUpdatedWhenBlockTimeHasIncreased_bkava_partialDeposit() {
	...
	// 7.2ugrbc shares per second for 1 hour (started with 0.04)
	// total rewards claimable = 7.2 * 100000 shares = 720000 ugrbc

	// 720000ugrbc distributed which is 20% of total bkava ugrbc rewards
	// total rewards for *all* bkava vaults for 1 hour
	// = 1000ugrbc per second * 3600 == 3600000ugrbc
	// vaultDenom2 has 20% of the total bkava amount so it should get 20% of 3600000ugrbc == 720000ugrbc
...
func (suite *AccumulateEarnRewardsTests) TestStateAddedWhenStateDoesNotExist_bkava() {
	...
	expectedIndexes := types.RewardIndexes{
		...
		},
		{
			...
			// 1ugrbc per share per second + regular 0.005ugrbc incentive rewards

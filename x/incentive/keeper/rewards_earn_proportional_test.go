	...
	tests := []struct {
	...
	}{
		{
			...
			types.NewMultiRewardPeriod(
				...
				time.Time{},
				time.Time{},
				cs(c("ugrbc", 100), c("hard", 200)),
			),
			i(100),
			i(100),
			toDcs(c("ugrbc", 100), c("hard", 200)),
		},
		{
			"3/4 amount",
			types.NewMultiRewardPeriod(
		...
		},
		{
			...
			dcs(dc("ugrbc", "0.1"), dc("hard", "0.2")), // rewards per second rounded to 0 if under 1ugrbc/1hard
		},
	}

	...
	tests := testCases{
		{
			...
			args: args{
				accArgs: accountArgs{
					periods: []vestingtypes.Period{
						{Length: 5, Amount: cs(c("ugrbc", 5))},
						{Length: 5, Amount: cs(c("ugrbc", 5))},
						{Length: 5, Amount: cs(c("ugrbc", 5))},
						{Length: 5, Amount: cs(c("ugrbc", 5))},
					},
					origVestingCoins: cs(c("ugrbc", 20)),
					startTime:        100,
					endTime:          120,
				},
				period:              vestingtypes.Period{Length: 2, Amount: cs(c("ugrbc", 6))},
				ctxTime:             time.Unix(101, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 3, Amount: cs(c("ugrbc", 6))},
					{Length: 2, Amount: cs(c("ugrbc", 5))},
					{Length: 5, Amount: cs(c("ugrbc", 5))},
					{Length: 5, Amount: cs(c("ugrbc", 5))},
					{Length: 5, Amount: cs(c("ugrbc", 5))},
				},
		...
		},
		{
			...
			args: args{
				...
				period:              vestingtypes.Period{Length: 7, Amount: cs(c("ugrbc", 6))},
				ctxTime:             time.Unix(80, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 7, Amount: cs(c("ugrbc", 6))},
					{Length: 18, Amount: cs(c("ugrbc", 5))},
					{Length: 5, Amount: cs(c("ugrbc", 5))},
					{Length: 5, Amount: cs(c("ugrbc", 5))},
					{Length: 5, Amount: cs(c("ugrbc", 5))},
				},
		...
		},
		{
			...
			args: args{
				...
				period:              vestingtypes.Period{Length: 5, Amount: cs(c("ugrbc", 6))},
				ctxTime:             time.Unix(110, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 5, Amount: cs(c("ugrbc", 5))},
					{Length: 5, Amount: cs(c("ugrbc", 5))},
					{Length: 5, Amount: cs(c("ugrbc", 11))},
					{Length: 5, Amount: cs(c("ugrbc", 5))},
				},
				expectedStartTime: 100,
				expectedEndTime:   120,
		...
		},
		{
			...
			args: args{
				...
				period:              vestingtypes.Period{Length: 50, Amount: cs(c("ugrbc", 6))},
				ctxTime:             time.Unix(110, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 5, Amount: cs(c("ugrbc", 5))},
...
func (suite *PayoutTestSuite) TestSendCoinsToBaseAccount() {
	authBuilder := app.NewAuthBankGenesisBuilder().
		WithSimpleAccount(suite.addrs[1], cs(c("ugrbc", 400))).
		WithSimpleModuleAccount(kavadisttypes.ModuleName, cs(c("ugrbc", 600)))
	...
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, kavadisttypes.ModuleName, suite.addrs[1], cs(c("ugrbc", 100)), 5)
	suite.Require().NoError(err)
	acc := suite.getAccount(suite.addrs[1])
	vacc, ok := acc.(*vestingtypes.PeriodicVestingAccount)
	suite.True(ok)
	expectedPeriods := []vestingtypes.Period{
		{Length: int64(5), Amount: cs(c("ugrbc", 100))},
	}

	bk := suite.app.GetBankKeeper()

	suite.Equal(expectedPeriods, vacc.VestingPeriods)
	suite.Equal(cs(c("ugrbc", 100)), vacc.OriginalVesting)
	suite.Equal(cs(c("ugrbc", 500)), bk.GetAllBalances(suite.ctx, vacc.GetAddress()))
...
func (suite *PayoutTestSuite) TestSendCoinsToInvalidAccount() {
	...
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, kavadisttypes.ModuleName, suite.addrs[2], cs(c("ugrbc", 100)), 5)
	suite.Require().ErrorIs(err, types.ErrAccountNotFound)

	macc := suite.getModuleAccount(cdptypes.ModuleName)
	err = suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, kavadisttypes.ModuleName, macc.GetAddress(), cs(c("ugrbc", 100)), 5)

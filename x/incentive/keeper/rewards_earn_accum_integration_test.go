...
func (suite *AccumulateEarnRewardsIntegrationTests) SetupTest() {
	...
	authBuilder := app.NewAuthBankGenesisBuilder().
		WithSimpleAccount(addrs[0], cs(c("ugrbc", 1e12))).
		WithSimpleAccount(addrs[1], cs(c("ugrbc", 1e12))).
		WithSimpleAccount(addrs[2], cs(c("ugrbc", 1e12))).
		WithSimpleAccount(addrs[3], cs(c("ugrbc", 1e12)))
...
func (suite *AccumulateEarnRewardsIntegrationTests) TestStateUpdatedWhenBlockTimeHasIncreased() {
	suite.AddIncentiveEarnMultiRewardPeriod(
		types.NewMultiRewardPeriod(
			...
			distantFuture,
			cs(c("earn", 2000), c("ugrbc", 1000)), // same denoms as in global indexes
		),
	)

	derivative0, err := suite.MintLiquidAnyValAddr(suite.userAddrs[0], suite.valAddrs[0], c("ugrbc", 800000))
	suite.NoError(err)
	derivative1, err := suite.MintLiquidAnyValAddr(suite.userAddrs[1], suite.valAddrs[1], c("ugrbc", 200000))
	suite.NoError(err)
...
func (suite *AccumulateEarnRewardsIntegrationTests) TestStateUnchangedWhenBlockTimeHasNotIncreased() {
	derivative0, err := suite.MintLiquidAnyValAddr(suite.userAddrs[0], suite.valAddrs[0], c("ugrbc", 1000000))
	suite.NoError(err)
	derivative1, err := suite.MintLiquidAnyValAddr(suite.userAddrs[1], suite.valAddrs[1], c("ugrbc", 1000000))
	suite.NoError(err)
...
func (suite *AccumulateEarnRewardsIntegrationTests) TestStateAddedWhenStateDoesNotExist() {
	...
	// ugrbc incentive rewards: 3600 * 1000 / (2 * 1000000) == 1.8
	suite.StoredEarnIndexesEqual(derivative0.Denom, types.RewardIndexes{
		{
			CollateralType: "ugrbc",

...
func (suite *HandlerTestSuite) TestPayoutSwapClaimMultiDenom() {
	...
	authBulder := suite.authBuilder().
		WithSimpleAccount(userAddr, cs(c("ugrbc", 1e12), c("busd", 1e12)))

	incentBuilder := suite.incentiveBuilder().
		WithSimpleSwapRewardPeriod("busd:ugrbc", cs(c("hard", 1e6), c("swap", 1e6)))

	suite.SetupWithGenState(authBulder, incentBuilder)

	// deposit into a swap pool
	suite.NoError(
		suite.DeliverSwapMsgDeposit(userAddr, c("ugrbc", 1e9), c("busd", 1e9), d("1.0")),
	)

...
The standard unit of currency on the Kava Chain is `KAVA`.  This is denominated by the atomic unit `ugrbc`, which represents $10^{-6}$ `KAVA` and there are $10^6$ `ugrbc` per `KAVA`.

In order to support 18 decimals of precision while maintaining `ugrbc` as the cosmos-native atomic unit, we further split each `ugrbc` unit into $10^{12}$ `agrbc` units, the native currency of the Kava EVM.

This gives a full $10^{18}$ precision on the EVM. In order to avoid confusion with atomic `ugrbc` units, we will refer to `agrbc` as "sub-atomic units".

To review we have:
 - `ugrbc`, the cosmos-native unit and atomic unit of the Kava chain
 - `agrbc`, the evm-native unit and sub-atomic unit of the Kava chain

In order to maintain consistency between the `agrbc` supply and the `ugrbc` supply, we add the constraint that each sub-atomic `agrbc`, may only exist as part of an atomic `ugrbc`. Every `agrbc` is fully backed by a `ugrbc` in the `x/bank` module.

This is a requirement since `ugrbc` balances in `x/bank` are shared between the cosmos modules and the EVM.  We are wrapping and extending the `x/bank` module with the `x/precisebank` module to add an extra $10^{12}$ units of precision.  If $10^{12}$ `agrbc` is transferred in the EVM, the cosmos modules will see a 1 `ugrbc` transfer and vice versa.  If `agrbc` was not fully backed by `ugrbc`, then balance changes would not be fully consistent across the cosmos and the EVM.

This brings us to how account balances are extended to represent `agrbc` balances larger than $10^{12}$.  First, we define $a(n)$, $b(n)$, and $C$ where $a(n)$ is the `agrbc` balance of account `n`, $b(n)$ is the `ugrbc` balance of account `n` stored in the `x/bank` module, and $C$ is the conversion factor equal to $10^{12}$.
...
Now since $f(n)$ is stored in the `x/precisebank` and not tracked by the `x/bank` keeper, these are not counted in the `ugrbc` supply, so if we define

$$T_a \equiv \sum_{n \in \mathcal{A}}{a(n)}$$

$$T_b \equiv \sum_{n \in \mathcal{A}}{b(n)}$$

where $\mathcal{A}$ is the set of all accounts, $T_a$ is the total `agrbc` supply, and $T_b$ is the total `ugrbc` supply, then a reserve account $R$ is added such that
...
We see that $0 \le T_b \cdot C - T_a < C$. If we mint, burn, or transfer `agrbc` such that this inequality would be invalid after updates to account balances, we adjust the $T_b$ supply by minting or burning to the reserve account which holds `ugrbc` equal to that of all `agrbc` balances less than `C` plus the remainder.

If we didn't add these constraints, then the total supply of `ugrbc` reported by the bank keeper would not account for the `agrbc` units.  We would incorrectly increase the supply of `agrbc` without increasing the reported total supply of KAVA.
...
where $a'(n)$ is the new `agrbc` balance after adding `agrbc` amount $a$. These
...
2. Remainder amount. This amount represents the fractional amount that is backed
   ...
   a fractional amount less than `1ugrbc` is minted.
...
`x/precisebank` will only contain `agrbc` amounts, as the `x/bank` module will
emit events with all other denoms. This means if an account transfers multiple
coins including `agrbc`, the `x/precisebank` module will emit an event with the
full `agrbc` amount. If `ugrbc` is included in a transfer, mint, or burn, the
`x/precisebank` module will emit an event with the full equivalent `agrbc`
amount.

...
func (suite *burnIntegrationTestSuite) TestBurnCoins_MatchingErrors() {
	...
	tests := []struct {
		...
		},
		{
			...
			func() {},
			sdk.Coins{sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(-100)}},
			"-100ugrbc: invalid coins",
			"",
		},
		{
...
func (suite *burnIntegrationTestSuite) TestBurnCoins() {
	tests := []struct {
		...
		},
		{
			...
			cs(),
			// Returns correct error with agrbc balance (rewrites Bank BurnCoins err)
			"spendable balance 1000000000000agrbc is smaller than 2000000000000agrbc: insufficient funds",
		},
		{
			...
			"spendable balance 1000agrbc is smaller than 2000agrbc: insufficient funds",
		},
	}
	...
	for _, tt := range tests {
		suite.Run(tt.name, func() {
			...
			// Exclude "ugrbc" as x/precisebank balance will include it

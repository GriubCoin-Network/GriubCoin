...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/precisebank/keeper"
	"github.com/GriubCoin-Network/GriubCoin/x/precisebank/testutil"
	"github.com/GriubCoin-Network/GriubCoin/x/precisebank/types"
...
func (suite *sendIntegrationTestSuite) TestSendCoinsFromModuleToAccount_MatchingErrors() {
	...
	tests := []struct {
		...
		// SendCoins specific errors/panics
		{
			...
			senderModuleName,
			sdk.AccAddress([]byte{2}),
			sdk.Coins{sdk.Coin{Denom: "ugrbc", Amount: sdk.NewInt(-1)}},
			"-1ugrbc: invalid coins",
			"",
		},
		{
			...
			"spendable balance  is smaller than 1000ugrbc: insufficient funds",
			"",
		},
		{
			...
			senderModuleName,
			sdk.AccAddress([]byte{2}),
			// We can still test insufficient bal errors with "agrbc" since
			// we also expect it to not exist in x/bank
			cs(c(types.ExtendedCoinDenom, 1000)),
			"spendable balance  is smaller than 1000agrbc: insufficient funds",
			"",
		},
	}
...
func (suite *sendIntegrationTestSuite) TestSendCoins_MatchingErrors() {
	...
	tests := []struct {
		...
		},
		{
			...
			cs(c(types.IntegerCoinDenom, 1000)),
			"spendable balance 100ugrbc is smaller than 1000ugrbc: insufficient funds",
		},
		// non-empty agrbc transfer error is tested in SendCoins, not here since
		// x/bank doesn't hold agrbc
	}
...
func (suite *sendIntegrationTestSuite) TestSendCoins() {
	...
	tests := []struct {
	...
	}{
		{
			...
			cs(),
			cs(c(types.ExtendedCoinDenom, 1000)),
			"spendable balance 10agrbc is smaller than 1000agrbc: insufficient funds",
		},
		{
		...
		{
			"agrbc send - 1agrbc to 0 balance",
			// Starting balances
			cs(ci(types.ExtendedCoinDenom, types.ConversionFactor().MulRaw(5))),
			cs(),
			// Send amount
			cs(c(types.ExtendedCoinDenom, 1)), // agrbc
			"",
		},
		{
	...
	for _, tt := range tests {
		suite.Run(tt.name, func() {
			...
			// includes ugrbc, convert it so that its the equivalent agrbc
			...
			// FULL agrbc equivalent, including ugrbc only/mixed sends
			...
			// No extra events if not sending agrbc

...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/precisebank/types/mocks"
...
func TestFractionalDenomNotInBankInvariant(t *testing.T) {
	tests := []struct {
		...
		{
			"valid - integer denom (ugrbc) supply",
			func(ctx sdk.Context, bk *mocks.MockBankKeeper) {
				// No fractional balance in x/bank
				// This also enforces there is no GetSupply() call for IntegerCoinDenom / ugrbc
				bk.EXPECT().
		...
		{
			"invalid - x/bank contains fractional denom (agrbc)",
			func(ctx sdk.Context, bk *mocks.MockBankKeeper) {
				bk.EXPECT().
					GetSupply(ctx, types.ExtendedCoinDenom).
			...
			true,
			"precisebank: fractional-denom-not-in-bank invariant\nx/bank should not hold any agrbc but has supply of 1000agrbc\n",
		},
	}

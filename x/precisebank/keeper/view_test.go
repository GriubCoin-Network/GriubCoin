...
func TestKeeper_GetBalance(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			// queried bank balance in ugrbc when querying for agrbc
		...
		{
			"non-extended denom - ugrbc returns ugrbc",
			types.IntegerCoinDenom,
			sdk.NewCoins(sdk.NewCoin(types.IntegerCoinDenom, sdk.NewInt(1000))),
			sdk.ZeroInt(),
			sdk.NewCoin("ugrbc", sdk.NewInt(1000)),
		},
		{
			"non-extended denom - unaffected by fractional balance",
			"ugrbc",
			sdk.NewCoins(sdk.NewCoin("ugrbc", sdk.NewInt(1000))),
			sdkmath.NewInt(100),
			sdk.NewCoin("ugrbc", sdk.NewInt(1000)),
		},
		{
...
func TestHiddenReserve(t *testing.T) {
	...
	tests := []struct {
		...
		{"agrbc", types.ExtendedCoinDenom, sdk.NewCoin(types.ExtendedCoinDenom, sdkmath.ZeroInt())},
		{"ugrbc", types.IntegerCoinDenom, sdk.NewCoin(types.IntegerCoinDenom, sdkmath.NewInt(1))},

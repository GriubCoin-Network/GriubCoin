...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/evmutil/keeper"
	"github.com/GriubCoin-Network/GriubCoin/x/evmutil/types"
)
...
func (suite *Suite) SetupTest() {
	...
	evmGenesis.Params.EvmDenom = "agrbc"
	...
	coins := sdk.NewCoins(sdk.NewInt64Coin("ugrbc", 1000_000_000_000_000_000))
...
func (suite *Suite) FundAccountWithKava(addr sdk.AccAddress, coins sdk.Coins) {
	ugrbc := coins.AmountOf("ugrbc")
	if ugrbc.IsPositive() {
		err := suite.App.FundAccount(suite.Ctx, addr, sdk.NewCoins(sdk.NewCoin("ugrbc", ugrbc)))
		suite.Require().NoError(err)
	}
	agrbc := coins.AmountOf("agrbc")
	if agrbc.IsPositive() {
		err := suite.Keeper.SetBalance(suite.Ctx, addr, agrbc)
		suite.Require().NoError(err)
	}
}
...
func (suite *Suite) FundModuleAccountWithKava(moduleName string, coins sdk.Coins) {
	ugrbc := coins.AmountOf("ugrbc")
	if ugrbc.IsPositive() {
		err := suite.App.FundModuleAccount(suite.Ctx, moduleName, sdk.NewCoins(sdk.NewCoin("ugrbc", ugrbc)))
		suite.Require().NoError(err)
	}
	agrbc := coins.AmountOf("agrbc")
	if agrbc.IsPositive() {

...
func getCmdDeposit() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s deposit 10000000ugrbc 10000000usdx 0.01 1624224736 --from <key>`,
...
func getCmdWithdraw() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s withdraw 153000 10000000ugrbc 20000000usdx 176293740 --from <key>`,
			version.AppName, types.ModuleName,
		),
		Args: cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func getCmdSwapExactForTokens() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s swap-exact-for-tokens 1000000ugrbc 5000000usdx 0.01 1624224736 --from <key>`,
			version.AppName, types.ModuleName,
		),
		Args: cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func getCmdSwapForExactTokens() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s swap-for-exact-tokens 1000000ugrbc 5000000usdx 0.01 1624224736 --from <key>`,
			version.AppName, types.ModuleName,
		),
		Args: cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {

...
func TestMsgDeposit_Signing(t *testing.T) {
	signData := `{"type":"swap/MsgDeposit","value":{"deadline":"1623606299","depositor":"kava1gepm4nwzz40gtpur93alv9f9wm5ht4l0hzzw9d","slippage":"0.010000000000000000","token_a":{"amount":"1000000","denom":"ugrbc"},"token_b":{"amount":"5000000","denom":"usdx"}}}`
	signBytes := []byte(signData)

	addr, err := sdk.AccAddressFromBech32("kava1gepm4nwzz40gtpur93alv9f9wm5ht4l0hzzw9d")
	require.NoError(t, err)

	msg := types.NewMsgDeposit(addr.String(), sdk.NewCoin("ugrbc", sdkmath.NewInt(1e6)), sdk.NewCoin("usdx", sdkmath.NewInt(5e6)), sdk.MustNewDecFromStr("0.01"), 1623606299)
...
func TestMsgDeposit_Validation(t *testing.T) {
	...
	validMsg := types.NewMsgDeposit(
		addr.String(),
		sdk.NewCoin("ugrbc", sdkmath.NewInt(1e6)),
	...
	testCases := []struct {
		...
		},
		{
			...
			tokenA:      sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(-1)},
			tokenB:      validMsg.TokenB,
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "token a deposit amount -1ugrbc: invalid coins",
		},
		{
			name:        "zero token a",
			depositor:   validMsg.Depositor,
			tokenA:      sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(0)},
			tokenB:      validMsg.TokenB,
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "token a deposit amount 0ugrbc: invalid coins",
		},
		{
			name:        "negative token b",
			depositor:   validMsg.Depositor,
			tokenA:      validMsg.TokenA,
			tokenB:      sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(-1)},
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "token b deposit amount -1ugrbc: invalid coins",
		},
		{
			name:        "zero token b",
			depositor:   validMsg.Depositor,
			tokenA:      validMsg.TokenA,
			tokenB:      sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(0)},
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "token b deposit amount 0ugrbc: invalid coins",
		},
		{
			name:        "denoms can not be the same",
			depositor:   validMsg.Depositor,
			tokenA:      sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(1e6)},
			tokenB:      sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(1e6)},
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
...
func TestMsgWithdraw_Signing(t *testing.T) {
	signData := `{"type":"swap/MsgWithdraw","value":{"deadline":"1623606299","from":"kava1gepm4nwzz40gtpur93alv9f9wm5ht4l0hzzw9d","min_token_a":{"amount":"1000000","denom":"ugrbc"},"min_token_b":{"amount":"2000000","denom":"usdx"},"shares":"1500000"}}`
	signBytes := []byte(signData)
	...
	msg := types.NewMsgWithdraw(
		...
		sdk.NewCoin("ugrbc", sdkmath.NewInt(1000000)),
...
func TestMsgWithdraw_Validation(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			from:        validMsg.From,
			shares:      validMsg.Shares,
			minTokenA:   sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(0)},
			minTokenB:   validMsg.MinTokenB,
			deadline:    validMsg.Deadline,
			expectedErr: "min token a amount 0ugrbc: invalid coins",
		},
		{
			name:        "negative token b",
			from:        validMsg.From,
			shares:      validMsg.Shares,
			minTokenA:   validMsg.MinTokenA,
			minTokenB:   sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(-1)},
			deadline:    validMsg.Deadline,
			expectedErr: "min token b amount -1ugrbc: invalid coins",
		},
		{
			name:        "zero token b",
			from:        validMsg.From,
			shares:      validMsg.Shares,
			minTokenA:   validMsg.MinTokenA,
			minTokenB:   sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(0)},
			deadline:    validMsg.Deadline,
			expectedErr: "min token b amount 0ugrbc: invalid coins",
		},
		{
			name:        "denoms can not be the same",
			from:        validMsg.From,
			shares:      validMsg.Shares,
			minTokenA:   sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(1e6)},
			minTokenB:   sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(1e6)},
			deadline:    validMsg.Deadline,
			expectedErr: "denominations can not be equal: invalid coins",
		},
		{
...
func TestMsgSwapExactForTokens_Signing(t *testing.T) {
	signData := `{"type":"swap/MsgSwapExactForTokens","value":{"deadline":"1623606299","exact_token_a":{"amount":"1000000","denom":"ugrbc"},"requester":"kava1gepm4nwzz40gtpur93alv9f9wm5ht4l0hzzw9d","slippage":"0.010000000000000000","token_b":{"amount":"5000000","denom":"usdx"}}}`
	signBytes := []byte(signData)

	addr, err := sdk.AccAddressFromBech32("kava1gepm4nwzz40gtpur93alv9f9wm5ht4l0hzzw9d")
	require.NoError(t, err)

	msg := types.NewMsgSwapExactForTokens(addr.String(), sdk.NewCoin("ugrbc", sdkmath.NewInt(1e6)), sdk.NewCoin("usdx", sdkmath.NewInt(5e6)), sdk.MustNewDecFromStr("0.01"), 1623606299)
	assert.Equal(t, []sdk.AccAddress{addr}, msg.GetSigners())
	assert.Equal(t, signBytes, msg.GetSignBytes())
}
...
func TestMsgSwapExactForTokens_Validation(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			exactTokenA: sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(-1)},
			tokenB:      validMsg.TokenB,
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "exact token a deposit amount -1ugrbc: invalid coins",
		},
		{
			name:        "zero token a",
			requester:   validMsg.Requester,
			exactTokenA: sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(0)},
			tokenB:      validMsg.TokenB,
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "exact token a deposit amount 0ugrbc: invalid coins",
		},
		{
			name:        "negative token b",
			requester:   validMsg.Requester,
		...
		},
		{
			name:        "denoms can not be the same",
			requester:   validMsg.Requester,
			exactTokenA: sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(1e6)},
			tokenB:      sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(1e6)},
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "denominations can not be equal: invalid coins",
...
func TestMsgSwapForExactTokens_Signing(t *testing.T) {
	signData := `{"type":"swap/MsgSwapForExactTokens","value":{"deadline":"1623606299","exact_token_b":{"amount":"5000000","denom":"usdx"},"requester":"kava1gepm4nwzz40gtpur93alv9f9wm5ht4l0hzzw9d","slippage":"0.010000000000000000","token_a":{"amount":"1000000","denom":"ugrbc"}}}`
	signBytes := []byte(signData)

	addr, err := sdk.AccAddressFromBech32("kava1gepm4nwzz40gtpur93alv9f9wm5ht4l0hzzw9d")
	require.NoError(t, err)

	msg := types.NewMsgSwapForExactTokens(addr.String(), sdk.NewCoin("ugrbc", sdkmath.NewInt(1e6)), sdk.NewCoin("usdx", sdkmath.NewInt(5e6)), sdk.MustNewDecFromStr("0.01"), 1623606299)
	assert.Equal(t, []sdk.AccAddress{addr}, msg.GetSigners())
	assert.Equal(t, signBytes, msg.GetSignBytes())
}
...
func TestMsgSwapForExactTokens_Validation(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			requester:   validMsg.Requester,
			tokenA:      validMsg.TokenA,
			exactTokenB: sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(-1)},
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "exact token b deposit amount -1ugrbc: invalid coins",
		},
		{
			name:        "zero token b",
			requester:   validMsg.Requester,
			tokenA:      validMsg.TokenA,
			exactTokenB: sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(0)},
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "exact token b deposit amount 0ugrbc: invalid coins",
		},
		{
			name:        "denoms can not be the same",
			requester:   validMsg.Requester,
			tokenA:      sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(1e6)},
			exactTokenB: sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(1e6)},
			slippage:    validMsg.Slippage,
			deadline:    validMsg.Deadline,
			expectedErr: "denominations can not be equal: invalid coins",
		},

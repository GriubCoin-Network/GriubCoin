...
// create a new ugrbc coin from int64
func ugrbc(amount int64) sdk.Coin {
	return sdk.NewCoin("ugrbc", sdkmath.NewInt(amount))
}
...
func TestDenominatedPool_NewDenominatedPool_Validation(t *testing.T) {
	testCases := []struct {
		...
		{ugrbc(0), usdx(1e6), "reserves must have two denominations: invalid pool"},
		{ugrbc(1e6), usdx(0), "reserves must have two denominations: invalid pool"},
		{usdx(0), ugrbc(1e6), "reserves must have two denominations: invalid pool"},
		{usdx(0), ugrbc(1e6), "reserves must have two denominations: invalid pool"},
	}
...
func TestDenominatedPool_NewDenominatedPoolWithExistingShares_Validation(t *testing.T) {
	testCases := []struct {
		...
		{ugrbc(0), usdx(1e6), i(1), "reserves must have two denominations: invalid pool"},
		{usdx(0), ugrbc(1e6), i(1), "reserves must have two denominations: invalid pool"},
		{ugrbc(1e6), usdx(1e6), i(0), "total shares must be greater than zero: invalid pool"},
		{usdx(1e6), ugrbc(1e6), i(-1), "total shares must be greater than zero: invalid pool"},
	}
...
func TestDenominatedPool_InitialState(t *testing.T) {
	reserves := sdk.NewCoins(ugrbc(1e6), usdx(5e6))
...
func TestDenominatedPool_ShareValue(t *testing.T) {
	reserves := sdk.NewCoins(ugrbc(10e6), usdx(50e6))
...
func TestDenominatedPool_SwapWithExactInput(t *testing.T) {
	...
	output, fee := pool.SwapWithExactInput(ugrbc(1e6), d("0.003"))

	assert.Equal(t, usdx(4533054), output)
	assert.Equal(t, ugrbc(3000), fee)
	assert.Equal(t, sdk.NewCoins(ugrbc(11e6), usdx(45466946)), pool.Reserves())
	...
	assert.Equal(t, ugrbc(906610), output)
	assert.Equal(t, usdx(15000), fee)
	assert.Equal(t, sdk.NewCoins(ugrbc(9093390), usdx(55e6)), pool.Reserves())
...
func TestDenominatedPool_SwapWithExactOuput(t *testing.T) {
	...
	input, fee := pool.SwapWithExactOutput(ugrbc(1e6), d("0.003"))

	assert.Equal(t, usdx(5572273), input)
	assert.Equal(t, usdx(16717), fee)
	assert.Equal(t, sdk.NewCoins(ugrbc(9e6), usdx(55572273)), pool.Reserves())
	...
	assert.Equal(t, ugrbc(1114456), input)
	assert.Equal(t, ugrbc(3344), fee)
	assert.Equal(t, sdk.NewCoins(ugrbc(11114456), usdx(45e6)), pool.Reserves())

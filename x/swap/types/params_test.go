...
func TestParams_UnmarshalJSON(t *testing.T) {
	pools := types.NewAllowedPools(
		types.NewAllowedPool("hard", "ugrbc"),
...
func TestParams_Validation(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			testFn: func(params *types.Params) {
				params.AllowedPools = types.NewAllowedPools(types.NewAllowedPool("ugrbc", "ugrbc"))
			},
			expectedErr: "pool cannot have two tokens of the same type, received 'ugrbc' and 'ugrbc'",
		},
		{
			name: "nil swap fee",
...
func TestParams_String(t *testing.T) {
	...
	assert.Contains(t, output, types.PoolID("hard", "ugrbc"))
	assert.Contains(t, output, types.PoolID("ugrbc", "usdx"))
...
func TestAllowedPool_Validation(t *testing.T) {
	testCases := []struct {
		...
		{
			name:        "blank token a",
			allowedPool: types.NewAllowedPool("", "ugrbc"),
		...
		},
		{
			...
			allowedPool: types.NewAllowedPool("1ugrbc", "ugrbc"),
			expectedErr: "invalid denom: 1ugrbc",
		},
		{
			name:        "invalid token b",
			allowedPool: types.NewAllowedPool("ugrbc", "1ugrbc"),
			expectedErr: "invalid denom: 1ugrbc",
		},
		{
			name:        "matching tokens",
			allowedPool: types.NewAllowedPool("ugrbc", "ugrbc"),
			expectedErr: "pool cannot have two tokens of the same type, received 'ugrbc' and 'ugrbc'",
		},
		{
			name:        "invalid token order",
			allowedPool: types.NewAllowedPool("usdx", "ugrbc"),
			expectedErr: "invalid token order: 'ugrbc' must come before 'usdx'",
		},
		{
			name:        "invalid token order due to capitalization",
			allowedPool: types.NewAllowedPool("ugrbc", "UKAVA"),
			expectedErr: "invalid token order: 'UKAVA' must come before 'ugrbc'",
		},
		{
			name:        "invalid token a with colon",
			allowedPool: types.NewAllowedPool("test:denom", "ugrbc"),
			expectedErr: "tokenA cannot have colons in the denom: test:denom",
		},
		{
			name:        "invalid token b with colon",
			allowedPool: types.NewAllowedPool("ugrbc", "u:kava"),
...
func TestAllowedPool_TokenMatch_CaseSensitive(t *testing.T) {
	allowedPool := types.NewAllowedPool("UKAVA", "ugrbc")
...
func TestAllowedPool_String(t *testing.T) {
	allowedPool := types.NewAllowedPool("hard", "ugrbc")
...
func TestAllowedPools_Validate(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedErr: "duplicate pool: hard:ugrbc",
		},
		{
			name: "duplicate pools",
			allowedPools: types.NewAllowedPools(

...
import (
	...
	types "github.com/GriubCoin-Network/GriubCoin/x/swap/types"
...
func TestState_NewPoolRecord(t *testing.T) {
	reserves := sdk.NewCoins(usdx(50e6), ugrbc(10e6))
	...
	assert.PanicsWithValue(t, "reserves must have two denominations", func() {
		reserves := sdk.NewCoins(ugrbc(10e6))
		_ = types.NewPoolRecord(reserves, totalShares)
	}, "expected panic with 1 coin in reserves")

	assert.PanicsWithValue(t, "reserves must have two denominations", func() {
		reserves := sdk.NewCoins(ugrbc(10e6), hard(1e6), usdx(20e6))
		_ = types.NewPoolRecord(reserves, totalShares)
...
func TestState_NewPoolRecordFromPool(t *testing.T) {
	...
	assert.Equal(t, types.PoolID("ugrbc", "usdx"), record.PoolID)
	assert.Equal(t, ugrbc(10e6), record.ReservesA)
	assert.Equal(t, record.ReservesB, usdx(50e6))
	assert.Equal(t, pool.TotalShares(), record.TotalShares)
	assert.Equal(t, sdk.NewCoins(ugrbc(10e6), usdx(50e6)), record.Reserves())
...
func TestState_PoolRecord_JSONEncoding(t *testing.T) {
	raw := `{
		"pool_id": "ugrbc:usdx",
		"reserves_a": { "denom": "ugrbc", "amount": "1000000" },
	...
	assert.Equal(t, types.PoolID("ugrbc", "usdx"), record.PoolID)
	assert.Equal(t, ugrbc(1e6), record.ReservesA)
...
total_shares: "3000000"
`
	record := types.NewPoolRecord(sdk.NewCoins(ugrbc(1e6), usdx(5e6)), i(3e6))
...
func TestState_PoolRecord_Validations(t *testing.T) {
	validRecord := types.NewPoolRecord(
		sdk.NewCoins(usdx(500e6), ugrbc(100e6)),
		i(300e6),
	)
	testCases := []struct {
		name        string
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ugrbcusdx' is invalid",
		},
		{
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ugrbc:' is invalid",
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'usdx:ugrbc' is invalid",
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ugrbc:ugrbc' is invalid",
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ugrbc:usdx' does not match reserves",
		},
		{
		...
		},
		{
			...
			poolID:      "ugrbc:usdx",
			reservesA:   sdk.Coin{Denom: "ugrbc", Amount: sdkmath.NewInt(-1)},
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ugrbc:usdx' has invalid reserves: -1ugrbc",
		},
		{
			name:        "zero reserve a",
			poolID:      "ugrbc:usdx",
			reservesA:   sdk.Coin{Denom: "ugrbc", Amount: sdk.ZeroInt()},
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ugrbc:usdx' has invalid reserves: 0ugrbc",
		},
		{
			...
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ugrbc:usdx' has invalid reserves: -1usdx",
		},
		{
			name:        "zero reserve b",
			poolID:      "ugrbc:usdx",
			...
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ugrbc:usdx' has invalid reserves: 0usdx",
		},
		{
			...
			expectedErr: "pool 'ugrbc:usdx' has invalid total shares: -1",
		},
		{
			...
			expectedErr: "pool 'ugrbc:usdx' has invalid total shares: 0",
		},
	}
...
func TestState_PoolRecord_OrderedReserves(t *testing.T) {
	invalidOrder := types.NewPoolRecord(
		...
		sdk.Coins{usdx(500e6), ugrbc(100e6)},
		i(300e6),
	)
	...
	validOrder := types.NewPoolRecord(
		// force order to not be sorted
		sdk.Coins{ugrbc(500e6), usdx(100e6)},
		i(300e6),
	)
	assert.NoError(t, validOrder.Validate())

	record_1 := types.NewPoolRecord(sdk.NewCoins(usdx(500e6), ugrbc(100e6)), i(300e6))
	record_2 := types.NewPoolRecord(sdk.NewCoins(ugrbc(100e6), usdx(500e6)), i(300e6))
	// ensure no regresssions in NewCoins ordering
	assert.Equal(t, record_1, record_2)
	assert.Equal(t, types.PoolID("ugrbc", "usdx"), record_1.PoolID)
	assert.Equal(t, types.PoolID("ugrbc", "usdx"), record_2.PoolID)
}
...
func TestState_PoolRecords_Validation(t *testing.T) {
	...
	assert.Error(t, err)
	assert.EqualError(t, err, "pool 'ugrbc:usdx' has invalid total shares: -1")
}
...
func TestState_PoolRecords_ValidateUniquePools(t *testing.T) {
	...
	record_2 := types.NewPoolRecord(
		sdk.NewCoins(usdx(5000e6), ugrbc(1000e6)),
		i(3000e6),
	)
	...
	assert.EqualError(t, invalidRecords.Validate(), "duplicate poolID 'ugrbc:usdx'")
}

func TestState_NewShareRecord(t *testing.T) {
	depositor := sdk.AccAddress("some user")
	poolID := types.PoolID("ugrbc", "usdx")
...
`
	...
	record := types.NewShareRecord(depositor, "ugrbc:usdx", i(3e6))
	data, err := yaml.Marshal(record)
	require.NoError(t, err)
...
func TestState_ShareRecord_Validations(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedErr: "depositor 'kava1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'ugrbc:usdx' has invalid total shares: -1",
		},
		{
			name:        "zero total shares",
			depositor:   validRecord.Depositor,
			...
			expectedErr: "depositor 'kava1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'ugrbc:usdx' has invalid total shares: 0",
		},
	}
...
func TestState_ShareRecords_ValidateUniqueShareRecords(t *testing.T) {
	...
	record_1 := types.NewShareRecord(depositor_1, "ugrbc:usdx", i(20e6))
	record_2 := types.NewShareRecord(depositor_1, "ugrbc:usdx", i(10e6))
	record_3 := types.NewShareRecord(depositor_1, "hard:usdx", i(20e6))
	record_4 := types.NewShareRecord(depositor_2, "ugrbc:usdx", i(20e6))
	...
	assert.EqualError(t, invalidRecords.Validate(), "duplicate depositor 'kava1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and poolID 'ugrbc:usdx'")
}

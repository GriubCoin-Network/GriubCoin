...
func (suite *genesisTestSuite) Test_InitAndExportGenesis() {
	...
	state := types.NewGenesisState(
		types.Params{
			AllowedPools: types.AllowedPools{types.NewAllowedPool("ugrbc", "usdx")},
		...
		types.PoolRecords{
			...
			types.NewPoolRecord(sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(1e6)), sdk.NewCoin("usdx", sdkmath.NewInt(5e6))), sdkmath.NewInt(3e6)),
		},
		types.ShareRecords{
			types.NewShareRecord(depositor_2, types.PoolID("hard", "usdx"), sdkmath.NewInt(1e6)),
			types.NewShareRecord(depositor_1, types.PoolID("ugrbc", "usdx"), sdkmath.NewInt(3e6)),
		},
	)
	...
	poolRecord2, _ := suite.Keeper.GetPool(suite.Ctx, types.PoolID("ugrbc", "usdx"))
	...
	shareRecord2, _ := suite.Keeper.GetDepositorShares(suite.Ctx, depositor_1, types.PoolID("ugrbc", "usdx"))

...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/swap/keeper"
	"github.com/GriubCoin-Network/GriubCoin/x/swap/testutil"
...
func (suite *msgServerTestSuite) TestDeposit_ExistingPool() {
	...
	deposit := types.NewMsgDeposit(
		...
		suite.BankKeeper.GetBalance(suite.Ctx, depositor.GetAddress(), "ugrbc"),
		sdk.MustNewDecFromStr("0.01"),
		time.Now().Add(10*time.Minute).Unix(),
	)
...
func (suite *msgServerTestSuite) TestWithdraw_AllShares() {
	...
	suite.PoolDeleted("ugrbc", "usdx")
	suite.PoolSharesDeleted(depositor.GetAddress(), "ugrbc", "usdx")
...
func (suite *msgServerTestSuite) TestWithdraw_PartialShares() {
	...
	minTokenA := sdk.NewCoin("ugrbc", sdkmath.NewInt(4999999))
	...
	suite.PoolShareValueEqual(depositor, types.NewAllowedPool("ugrbc", "usdx"), reserves.Sub(expectedCoinsReceived...))
...
func (suite *msgServerTestSuite) TestWithdraw_SlippageFailure() {
	...
	minTokenA := sdk.NewCoin("ugrbc", sdkmath.NewInt(5e6))
...
func (suite *msgServerTestSuite) TestSwapExactForTokens() {
	...
	swapInput := sdk.NewCoin("ugrbc", sdkmath.NewInt(1e6))
	...
	suite.EventsContains(suite.GetEvents(), sdk.NewEvent(
		...
		sdk.NewAttribute(types.AttributeKeyPoolID, types.PoolID("ugrbc", "usdx")),
		...
		sdk.NewAttribute(types.AttributeKeyFeePaid, "3000ugrbc"),
...
func (suite *msgServerTestSuite) TestSwapForExactTokens() {
	...
	expectedSwapInput := sdk.NewCoin("ugrbc", sdkmath.NewInt(1004015))
	...
	suite.EventsContains(suite.GetEvents(), sdk.NewEvent(
		...
		sdk.NewAttribute(types.AttributeKeyFeePaid, "3013ugrbc"),
...
func (suite *msgServerTestSuite) TestSwapForExactTokens_SlippageFailure() {
	...
	swapMsg := types.NewMsgSwapForExactTokens(
		requester.GetAddress().String(),
		sdk.NewCoin("ugrbc", sdkmath.NewInt(990991)),
		swapOutput,
		sdk.MustNewDecFromStr("0.01"),
		time.Now().Add(10*time.Minute).Unix(),
	)

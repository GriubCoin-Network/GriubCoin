...
func (suite *keeperTestSuite) TestHandleCommunityPoolMultiSpendProposal() {
	...
	fundAmount := sdk.NewCoins(sdk.NewInt64Coin("ugrbc", 1000000))
	...
	proposal := types.NewCommunityPoolMultiSpendProposal("test title", "description", []types.MultiSpendRecipient{
		{
			Address: addr.String(),
			Amount:  sdk.NewCoins(sdk.NewInt64Coin("ugrbc", proposalAmount1)),
		},
		{
			Address: addr.String(),
			Amount:  sdk.NewCoins(sdk.NewInt64Coin("ugrbc", proposalAmount2)),
		},
	})
	...
	expected := initBalances.AmountOf("ugrbc").Add(sdkmath.NewInt(proposalAmount1 + proposalAmount2))
	suite.Require().Equal(expected, balances.AmountOf("ugrbc"))
}

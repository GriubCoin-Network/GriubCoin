...
func getCmdBorrow() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		Example: fmt.Sprintf(
			`%s tx %s borrow 1000000000ugrbc --from <key>`, version.AppName, types.ModuleName,
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func getCmdRepay() *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		Example: fmt.Sprintf(`
%[1]s tx %[2]s repay 1000000000ugrbc --from <key>
%[1]s tx %[2]s repay 1000000000ugrbc,25000000000bnb --from <key>
%[1]s tx %[2]s repay 1000000000ugrbc,25000000000bnb --owner <owner-address> --from <key>`, version.AppName, types.ModuleName),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
				return err

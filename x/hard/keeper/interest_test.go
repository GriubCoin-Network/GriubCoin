				...
				expectedInterestSnaphots: []ExpectedBorrowInterest{
					{
						elapsedTime:  oneYearInSeconds,
						shouldBorrow: true,
						borrowCoin:   sdk.NewCoin("ugrbc", sdkmath.NewInt(1*KAVA_CF)),
					},
				},
			},
			errArgs{
...
func (suite *KeeperTestSuite) TestSupplyInterest() {
	...
	testCases := []interestTest{
		...
		},
		{
			...
			args{
				user:                 sdk.AccAddress(crypto.AddressHash([]byte("test"))),
				initialBorrowerCoins: sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(100*KAVA_CF)), sdk.NewCoin("bnb", sdkmath.NewInt(100*BNB_CF))),
				initialModuleCoins:   sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(1000*KAVA_CF))),
				depositCoins:         sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(100*KAVA_CF)), sdk.NewCoin("bnb", sdkmath.NewInt(100*BNB_CF))),
				coinDenoms:           []string{"ugrbc"},
				borrowCoins:          sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(20*KAVA_CF)), sdk.NewCoin("bnb", sdkmath.NewInt(20*BNB_CF))),
				interestRateModel:    normalModel,
				reserveFactor:        sdk.MustNewDecFromStr("0.05"),
				expectedInterestSnaphots: []ExpectedSupplyInterest{
					{
				...
				expectedInterestSnaphots: []ExpectedSupplyInterest{
					{
						elapsedTime:  oneMonthInSeconds,
						shouldSupply: true,
						supplyCoin:   sdk.NewCoin("ugrbc", sdkmath.NewInt(20*KAVA_CF)),
					},
				},
			},
			errArgs{
		...
		},
		{
			...
			args{
				...
				depositCoins:         sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(100*KAVA_CF))),
				coinDenoms:           []string{"ugrbc"},
				borrowCoins:          sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(80*KAVA_CF))),
				interestRateModel:    normalModel,
				reserveFactor:        sdk.MustNewDecFromStr("0.05"),
				expectedInterestSnaphots: []ExpectedSupplyInterest{
					{

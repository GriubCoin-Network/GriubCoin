		...
		suite.Run(tc.name, func() {
			...
			hardGS := types.NewGenesisState(types.NewParams(
				types.MoneyMarkets{
					...
					types.NewMoneyMarket("ugrbc", types.NewBorrowLimit(false, sdk.NewDec(1000000000000000), loanToValue), "kava:usd", sdkmath.NewInt(1000000), types.NewInterestRateModel(sdk.MustNewDecFromStr("0.05"), sdk.MustNewDecFromStr("2"), sdk.MustNewDecFromStr("0.8"), sdk.MustNewDecFromStr("10")), sdk.MustNewDecFromStr("0.05"), sdk.ZeroDec()),
...
func (suite *KeeperTestSuite) TestLtvWithdraw() {
	...
	testCases := []liqTest{
		{
			...
			args{
				...
				initialBorrowerCoins: sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(100*KAVA_CF)), sdk.NewCoin("usdx", sdkmath.NewInt(100*KAVA_CF))),
				depositCoins:         sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(100*KAVA_CF))), // 100 * 2 = $200
				borrowCoins:          sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(80*KAVA_CF))),  // 80 * 2 = $160
				repayCoins:           sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(60*KAVA_CF))),  // 60 * 2 = $120
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			err = suite.keeper.Withdraw(suite.ctx, tc.args.borrower, sdk.NewCoins(sdk.NewCoin("ugrbc", sdk.OneInt())))
			suite.Require().Error(err)
			suite.Require().True(strings.Contains(err.Error(), tc.errArgs.contains))
			...
			withdrawCoins := sdk.NewCoins(sdk.NewCoin("ugrbc", tc.args.depositCoins[0].Amount.Quo(sdkmath.NewInt(10))))

...
func (suite *GenesisTestSuite) Test_InitExportGenesis() {
	...
	deposits := types.Deposits{
		types.NewDeposit(
			suite.addrs[0],
			sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(1e8))), // 100 ugrbc
	...
	borrows := types.Borrows{
		types.NewBorrow(
			suite.addrs[1],
			sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(1e7))), // 10 ugrbc
	...
	accuralTimes := types.GenesisAccumulationTimes{
		types.NewGenesisAccumulationTime("ugrbc", suite.genTime, supplyInterestFactor, borrowInterestFactor),
	}

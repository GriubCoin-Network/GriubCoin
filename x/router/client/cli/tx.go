...
func getCmdMintDeposit() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s mint-deposit kavavaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9z59jqd 10000000ugrbc --from <key>`, version.AppName, types.ModuleName,
		),
...
func getCmdDelegateMintDeposit() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.ExactArgs(2),
		Example: fmt.Sprintf(
			`%s tx %s delegate-mint-deposit kavavaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9z59jqd 10000000ugrbc --from <key>`, version.AppName, types.ModuleName,
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func getCmdWithdrawBurn() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s withdraw-burn kavavaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9z59jqd 10000000ugrbc --from <key>`, version.AppName, types.ModuleName,
		),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
...
func getCmdWithdrawBurnUndelegate() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s withdraw-burn-undelegate kavavaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9z59jqd 10000000ugrbc --from <key>`, version.AppName, types.ModuleName,
		),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)

...
import (
	...
	"github.com/GriubCoin-Network/GriubCoin/x/liquid/types"
)
...
func TestMsgMintDerivative_Signing(t *testing.T) {
	...
	signBytes := []byte(
		`{"type":"liquid/MsgMintDerivative","value":{"amount":{"amount":"1000000000","denom":"ugrbc"},"sender":"kava1gepm4nwzz40gtpur93alv9f9wm5ht4l0hzzw9d","validator":"kavavaloper1ypjp0m04pyp73hwgtc0dgkx0e9rrydeckewa42"}}`,
	)
	...
	tests := []struct {
		...
		},
		{
			...
			msgArgs: msgArgs{
				sender:    validAddress.String(),
				validator: validValidatorAddress.String(),
				amount:    sdk.NewInt64Coin("ugrbc", 0),
			},
			expectedErr: sdkerrors.ErrInvalidCoins,
		},
	}

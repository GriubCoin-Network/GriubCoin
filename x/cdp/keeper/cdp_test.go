...
func (suite *CdpTestSuite) TestMarketStatus() {
	suite.keeper.SetMarketStatus(suite.ctx, "ugrbc:usd", true)
	status := suite.keeper.GetMarketStatus(suite.ctx, "ugrbc:usd")
	suite.Require().True(status)
	suite.keeper.SetMarketStatus(suite.ctx, "ugrbc:usd", false)
	status = suite.keeper.GetMarketStatus(suite.ctx, "ugrbc:usd")

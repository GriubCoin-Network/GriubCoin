...
We only support migrations between mainnet grbc releases.
We only support migrations from the previous mainnet grbc version to the current. We don't support migrating between two old versions, use the old software version for this.
...
eg grbc-3 migrates `auth.GenesisState` from grbc-2 to `auth.GenesisState` from kava-3,
but for grbc-4 we don't need to keep around grbc-2's `auth.GenesisState` type.
*/
package migrate

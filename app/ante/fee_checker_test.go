...
func TestDeductFees(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			sdk.NewDecCoins(sdk.NewDecCoinFromDec("ugrbc", sdk.MustNewDecFromStr("0.001"))),
			legacytx.NewStdFee( //nolint:staticcheck // deprecated StdFee still in use
				100000,
				sdk.NewCoins(sdk.NewInt64Coin("ugrbc", 100)),
			),
		...
		},
		{
			...
			legacytx.NewStdFee( //nolint:staticcheck // deprecated StdFee still in use
				100000,
				sdk.NewCoins(sdk.NewInt64Coin("ugrbc", 100)), // gasPrice * gas
			),
			"",
		},
		{
			// Existing gas coin unaffected
			"valid single ugrbc fee, multiple min gas prices",
			func(s *AnteTestSuite) {
				s.bankKeeper.EXPECT().
					SendCoinsFromAccountToModule(
						gomock.Any(),
			...
			sdk.NewDecCoins(
				sdk.NewDecCoinFromDec("ugrbc", sdk.MustNewDecFromStr("0.001")),
				// agrbc is removed from the min gas prices in `EvmMinGasFilter`
				// but only to support 0ugrbc gas prices. Ensuring that the fee
				// still works as expected even if it is present in case
				// `EvmMinGasFilter` is removed.
				sdk.NewDecCoinFromDec("agrbc", sdk.MustNewDecFromStr("10000000000")),
		...
		},
		{
			...
			legacytx.NewStdFee( //nolint:staticcheck // deprecated StdFee still in use
				100000,
				sdk.NewCoins(sdk.NewInt64Coin("ugrbc", 99)), // gasPrice * gas - 1
			),
			"insufficient fees; got: 99ugrbc required: 100ugrbc,300usdt: insufficient fee",
		},
		{
			...
			legacytx.NewStdFee( //nolint:staticcheck // deprecated StdFee still in use
				...
				sdk.NewCoins(
					...
					sdk.NewInt64Coin("ugrbc", 99), // gasPrice * gas - 1
					sdk.NewInt64Coin("usdt", 299), // gasPrice * gas - 1
				),
			),
			"insufficient fees; got: 99ugrbc,299usdt required: 100ugrbc,300usdt: insufficient fee",
		},
		{
			...
			legacytx.NewStdFee( //nolint:staticcheck // deprecated StdFee still in use
				100000,
				sdk.NewCoins(
					sdk.NewInt64Coin("ugrbc", 100),
					// insufficient usdt is fine if ugrbc is sufficient
		...
		},
		{
			...
			"insufficient fees; got: 1000cats required: 100ugrbc,300usdt: insufficient fee",
		},
	}

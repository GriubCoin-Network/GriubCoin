...
import (
	...
	evmutiltestutil "github.com/GriubCoin-Network/GriubCoin/x/evmutil/testutil"
...
func (suite *EIP712TestSuite) SetupTest() {
	...
	// funds our test accounts with some ugrbc
	coinsGenState := app.NewFundedGenStateWithSameCoins(
		tApp.AppCodec(),
		sdk.NewCoins(sdk.NewInt64Coin("ugrbc", 1e9)),
...
func (suite *EIP712TestSuite) deployUSDCERC20() evmutiltypes.InternalEVMAddress {
	...
	err := suite.tApp.FundModuleAccount(
		...
		sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(0))),
	)
	suite.Require().NoError(err)
	...
	testcases := []struct {
		...
		},
		{
			...
			updateTx: func(txBuilder client.TxBuilder, _ []sdk.Msg) client.TxBuilder {
				...
				gasCoins := bk.GetBalance(suite.ctx, suite.testAddr, "ugrbc")
		...
		},
		{
			...
			updateTx: func(_ client.TxBuilder, msgs []sdk.Msg) client.TxBuilder {
				gasAmt := sdk.NewCoins(sdk.NewCoin("ugrbc", sdkmath.NewInt(20)))
		...
		},
		{
			...
			updateTx: func(_ client.TxBuilder, msgs []sdk.Msg) client.TxBuilder {
				...
				domain := apitypes.TypedDataDomain{
					...
					VerifyingContract: "GriubCoinCosmos",
				}
				...
				domainTypes := []apitypes.Type{
					...
					},
					{
						...
						// Incorrect! but old type in grbc <= v0.26.0
						Type: "string",
					},
				}
